...
def preprocess_image(image_data):
    ...
    encoded_prefix = "{'encoded_image': 'data:image/png;base64,"
    print('old image_data: ', image_data[:80])
    if image_data.startswith(encoded_prefix):
        
        image_data = image_data[len(encoded_prefix):]
        print('new image_data: ', image_data[:80])
...
def main(sample_event):
    try:
        print("entrou no Try")
        
        print('sample_event: ', str(sample_event)[:80])

        predicted_letters = preprocess_image(str(sample_event))
