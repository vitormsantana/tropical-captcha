...
getButtons(){
    ...
    addCartItem(cartItem) {
        ...
        cartItemUi.innerHTML = `
            <div class="cart-product">
                ...
                <div class="plus-minus">
                    <i class="fa fa-angle-left add-ammount" data-id="${cartItem.id}"></i>
                    <span class="no-of-items" data-id="${cartItem.id}">${cartItem.amount}</span>
                </div>
            </div>
        `;
        cartContent.append(cartItemUi); // Make sure you have an element with class "cart-content"
    }
		setupApp(){
		...
		else if(event.target.classList.contains("add-ammount")){
			let id = event.target.dataset.id
		...
		else if(event.target.classList.contains("reduce-ammount")){
			let id = event.target.dataset.id
			let item = Cart.find((item)=>item.id===id)
			if(item.amount>1){
				item.amount--
    ...
    addAmount() {
        const addBtn = document.querySelectorAll(".add-ammount"); // Correct class name
        addBtn.forEach((btn) => {
