...
for model_name in model_names:
    ...
    # Create a dictionary to store the misclassified occurrences per character
    misclassified_occurrences = {label: 0 for label in class_labels}
    
    # Count the misclassified occurrences for each character
    for true_label, pred_label in zip(Y_true_labels, Y_pred_labels):
        if true_label != pred_label:
            true_char = lb.classes_[true_label]
            misclassified_occurrences[true_char[0]] += 1
    
    # Print the table
    print("Misclassified Occurrences per Character:")
    print("Character\tMisclassified Count")
    print("---------------------------------")
    for char, count in misclassified_occurrences.items():
        print(f"{char}\t\t{count}")
        
    # Count the correctly predicted words (100% correct)
    correctly_predicted_words = 0
    total_words = len(val_data.classes) // 6  # Assuming there are 6 characters in each word
    for i in range(0, len(val_data.classes), 6):
        word_true_labels = Y_true_labels[i:i+6]
        word_pred_labels = Y_pred_labels[i:i+6]
        if all(true_label == pred_label for true_label, pred_label in zip(word_true_labels, word_pred_labels)):
            correctly_predicted_words += 1
    
    # Calculate the percentage of total words that were 100% correctly predicted
    percentage_correct_words = (correctly_predicted_words / total_words) * 100
    print("Number of Correctly Predicted Words (100% correct):", correctly_predicted_words)
    print("Percentage of Total Words 100% Correct:", percentage_correct_words, "%")