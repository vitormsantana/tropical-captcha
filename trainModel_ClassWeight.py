...
class_indices = np.unique(train_data.classes)
class_weights = compute_class_weight(class_weight = 'balanced',classes = class_indices,y = train_data.classes)
...
history = model.fit(train_data, validation_data=val_data, epochs=17, verbose=1, class_weight=class_weight_dict)
...
plt.xticks(class_indices, class_labels, rotation='vertical')
plt.tight_layout()
plt.show()
print(class_weights)