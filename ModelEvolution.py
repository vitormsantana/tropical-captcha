...
# Define the range of training image counts
training_counts = np.arange(10, len(X_train) + 1, 20)  # Adjust the range of training_counts
accuracy_values = []

for num_images in training_counts:
    if num_images <= len(X_train):
        print(f"Training with {num_images} images...")
        train_subset = np.random.choice(len(X_train), num_images, replace=False)
        model.fit(X_train[train_subset], Y_train[train_subset], epochs=30, verbose=0)

        # Evaluate the model on the test data
        evaluation = model.evaluate(X_test, Y_test, verbose=0)
        accuracy_values.append(evaluation[1])
    else:
        print(f"Skipping training with {num_images} images as it exceeds the available data.")
